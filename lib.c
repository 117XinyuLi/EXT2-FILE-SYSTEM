...
void open_file(char tmp[100]) {
    ...
    if (flag) {
        if (search_file_in_open_table(dir_buf[k].inode)) {
            ...
            fopen_table[flag] = (short) dir_buf[k].inode;

            // 更新文件的访问时间
            load_inode_entry(dir_buf[k].inode);
            time_t t;
            time(&t);
            inode_buf.i_atime = t;
            update_inode_entry(dir_buf[k].inode);
...
void close_file(char tmp[100]) {
    ...
    if (flag) {
        if (search_file_in_open_table(dir_buf[k].inode)) {
            flag = 0;
            while (fopen_table[flag] != dir_buf[k].inode) {
                flag++;
            }
            fopen_table[flag] = 0;
...
void read_file(char tmp[100]) {
    ...
    flag = find_file(tmp, 1, &i, &j, &k);
    if (flag) {
        if (search_file_in_open_table(dir_buf[k].inode)) //读文件的前提是该文件已经打开
        {
            load_inode_entry(dir_buf[k].inode);
            // 修改文件的访问时间
            time_t t;
            ...
            if (inode_buf.i_blocks <= 6){
                for (flag = 0; flag < inode_buf.i_blocks; flag++) {
                    load_block(inode_buf.i_block[flag]);
                ...
                for (int i = 0; i < 64; i++) {
                    for (int j = 0; j < 8; j++) {
                        if (inode_ptr_buf[i].i_block[j] != 0) {
                            load_block(inode_ptr_buf[i].i_block[j]);
                            for (t = 0; t < 512; t++) {
                                printf("%c", Buffer[t]);
                                size--;
                                if (size == 0){
                    ...
                    for (int j = 0; j < 8; j++) {
                        if (inode_ptr_buf[i].i_block[j] != 0) {
                            ...
                            for (int k = 0; k < 64; k++) {
                                for (int l = 0; l < 8; l++) {
                                    if (inode_ptr_buf_level_2[k].i_block[l] != 0) {
                                        load_block(inode_ptr_buf_level_2[k].i_block[l]);
                                        for (t = 0; t < 512; t++) {
                                            printf("%c", Buffer[t]);
                                            size--;
                                            if (size == 0){
        ...
        if (search_file_in_open_table(dir_buf[k].inode)) {
            ...
            if (need_blocks <= 6) // 直接索引，使用6个直接索引块
            {
                ...
                if (inode_buf.i_blocks <= need_blocks) {
                    while (inode_buf.i_blocks < need_blocks) {
                        ...
                        {
                            for (int i = 0; i < inode_buf.i_blocks; i++) {
                                remove_block(inode_buf.i_block[i]);
                            }
                ...
                while (j < need_blocks) {
                    if (j != need_blocks - 1) {
                        load_block(inode_buf.i_block[j]);
            ...
            else if(need_blocks <= 6 + 256 + 256 * 256) // 二次间接索引，使用6个直接索引块，1个一次间接索引块和1个二次间接索引块
            {
                ...
                for (int i = 0; i < needed_index_blocks; i++){
                    ...
                    {
                        for (int j = 0; j < i; j++){
                            remove_block(inode_index_2_index[j]);
                        }
                        printf("No enough space!\n");
                        return 2;
                    }
...
int get_dir_size(short inode_num) {
    ...
    for (int i = 0; i < num_of_files; i++) {
        load_inode_entry(inode_num);
        load_dir(inode_buf.i_block[i/32]);
        if (dir_buf[i%32].file_type == 2 && strcmp(dir_buf[i%32].name, ".") && strcmp(dir_buf[i%32].name, "..")) {
            size += get_dir_size(dir_buf[i%32].inode) + 0;
        }
        else if (!strcmp(dir_buf[i].name, ".") || !strcmp(dir_buf[i].name, "..")) {
            size += 0;
        }
        else {
            load_inode_entry(dir_buf[i%32].inode);
...
void ls() {
    ...
    while (i < inode_buf.i_blocks) {
        ...
        while (k < 32) {
            if (dir_buf[k].inode) {
                printf("%-10s\t", dir_buf[k].name);
                if (dir_buf[k].file_type == 2) {
                    j = 0;
                    load_inode_entry(dir_buf[k].inode);
                    if (!strcmp(dir_buf[k].name, "..")) {
                        flag = 1;
                    }
                    else if (!strcmp(dir_buf[k].name, ".")) {
                        flag = 0;
                    }
                    else {
                        flag = 2;
                    ...
                    if (flag != 2) {
                        ...
                        long long size = get_dir_size(dir_buf[k].inode);
